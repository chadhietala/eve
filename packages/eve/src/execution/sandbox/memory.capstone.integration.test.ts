 ...
 * path under the memory root is readable through the real `read_file` tool in a
 * later turn — modeled here as a fresh context with a brand-new store instance
 * pointing at the same directory (i.e. it survives process/session boundaries).
 *
 * The sandbox throws on any access, so a passing test also proves the file
...
describe("memory layer end-to-end (file tools → store)", () => {
  it("a named file written to /memory in one turn is readable in a later turn", async () => {
    ...
    try {
      ...
      await inTurn(configFor(new FsMemoryStore(dir)), async () => {
        const result = await executeWriteFileOnSandbox(sandboxNeverTouched, {
          ...
          filePath: "/memory/index.md",
        });
        expect(result.existed).toBe(false);
      });

      // Turn 2 — brand-new context AND a new store instance over the same dir,

...
describe("InMemoryMemoryStore versioning", () => {
  it("records a version on a write and another on a content change, newest-first", async () => {
    let t = 0;
    const store = new InMemoryMemoryStore(() => `2026-06-19T00:00:0${t++}.000Z`);
    await store.write(ns, "notes.md", bytes("v1"), "k1");
    await store.write(ns, "notes.md", bytes("v2"), "k2");

    const versions = await store.listVersions(ns, "notes.md");
  ...
  it("returns no versions for an absent path", async () => {
    const store = new InMemoryMemoryStore();
    expect(await store.listVersions(ns, "missing.md")).toEqual([]);
  });

  it("readVersion returns the bytes of an old version", async () => {
    const store = new InMemoryMemoryStore();
    await store.write(ns, "notes.md", bytes("old"), "k1");
    await store.write(ns, "notes.md", bytes("new"), "k2");

    expect(text(await store.readVersion(ns, "notes.md", sha256("old")))).toBe("old");
    expect(text(await store.read(ns, "notes.md"))).toBe("new");
  });

  it("readVersion returns null for an unknown version", async () => {
    const store = new InMemoryMemoryStore();
    await store.write(ns, "notes.md", bytes("x"), "k1");
    expect(await store.readVersion(ns, "notes.md", sha256("nope"))).toBeNull();
  });
  ...
  it("restore = read an old version and write it back, recording a fresh head version", async () => {
    ...
    const restored = await store.readVersion(ns, "notes.md", sha256("v1"));
    expect(restored).not.toBeNull();
    await store.write(ns, "notes.md", restored as Uint8Array, "k3");

    expect(text(await store.read(ns, "notes.md"))).toBe("v1");
  ...
  it("writing identical content under a fresh key records no new version", async () => {
    const store = new InMemoryMemoryStore();
    await store.write(ns, "notes.md", bytes("same"), "k1");
    await store.write(ns, "notes.md", bytes("same"), "k2");

    expect((await store.listVersions(ns, "notes.md")).length).toBe(1);
  });
});
...
describe("InMemoryMemoryStore compare-and-swap", () => {
  it("succeeds when expectedVersion matches the head", async () => {
    const store = new InMemoryMemoryStore();
    await store.write(ns, "notes.md", bytes("v1"), "k1");
    await store.write(ns, "notes.md", bytes("v2"), "k2", { expectedVersion: sha256("v1") });
    expect(text(await store.read(ns, "notes.md"))).toBe("v2");
  });
  ...
  it("throws MemoryConflictError on a stale expectedVersion, carrying expected/actual", async () => {
    ...
    let caught: unknown;
    try {
      await store.write(ns, "notes.md", bytes("v3"), "k3", { expectedVersion: stale });
    } catch (error) {
      caught = error;
    }
  ...
  it("expectedVersion null succeeds on an absent path", async () => {
    const store = new InMemoryMemoryStore();
    await store.write(ns, "fresh.md", bytes("hi"), "k1", { expectedVersion: null });
    expect(text(await store.read(ns, "fresh.md"))).toBe("hi");
  });

  it("expectedVersion null conflicts on an existing path", async () => {
    const store = new InMemoryMemoryStore();
    await store.write(ns, "notes.md", bytes("here"), "k1");
    await expect(
      store.write(ns, "notes.md", bytes("clobber"), "k2", { expectedVersion: null }),
  ...
  it("a CAS conflict does not consume the write key, so a retry can still apply", async () => {
    const store = new InMemoryMemoryStore();
    await store.write(ns, "notes.md", bytes("head"), "k1");

    await expect(
      store.write(ns, "notes.md", bytes("v3"), "kRetry", { expectedVersion: sha256("stale") }),
    ).rejects.toBeInstanceOf(MemoryConflictError);

    // Same key, now with the correct expectation, must apply.
    await store.write(ns, "notes.md", bytes("v3"), "kRetry", { expectedVersion: sha256("head") });
    expect(text(await store.read(ns, "notes.md"))).toBe("v3");
  });
});

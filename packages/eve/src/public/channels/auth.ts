 ...
 * Connect calls to reuse the correct user-scoped authorization. Every other
 * request returns `null`, skipping to the next entry under
...
async function resolveLocalDevSessionAuthContext(request: Request): Promise<SessionAuthContext> {
  ...
  const authServer = LocalDevelopmentAuthServer.fromEnvironment();

...
function run(input: {
  command: "vc" | "login" | "model" | "channels" | "connect" | "deploy";
  flows: TuiSetupFlows;
...
describe("runTuiSetupCommand", () => {
  ...
  it("routes a Vercel login action raised by the connections flow to /login", async () => {
    const flows = fakeFlows({
      runConnectionsFlow: vi.fn<TuiSetupFlows["runConnectionsFlow"]>(async () => {
    ...
    await expect(run({ command: "connect", flows })).resolves.toEqual({
      message: "You're not logged in to Vercel — run /login, then retry /connect.",
      preserveFlowDiagnostics: true,
    });
  });

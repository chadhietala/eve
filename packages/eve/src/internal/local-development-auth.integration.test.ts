...
describe("LocalDevelopmentAuthServer", () => {
  it("resolves an immutable TUI grant through worker-facing server coordinates", async () => {
    ...
    const writer = LocalDevelopmentAuthServer.fromMetadata({
      appRoot,
      metadata: handle.metadata,
    });
    const reader = LocalDevelopmentAuthServer.fromEnvironment();
  ...
  it("rejects forged credentials and grants from another server instance", async () => {
    ...
    const firstServer = LocalDevelopmentAuthServer.fromMetadata({
      appRoot,
      metadata: firstHandle.metadata,
    });
    const firstGrant = resultValue(await firstServer.create({ userId: "vercel-user-123" }));

    await expect(firstServer.read("forged-credential")).resolves.toEqual({
      ok: true,
      value: undefined,
    });
    ...
    const secondServer = LocalDevelopmentAuthServer.fromMetadata({
      appRoot,
      metadata: secondHandle.metadata,
    });
    await expect(secondServer.read(firstGrant.token)).resolves.toEqual({
      ok: true,
      value: undefined,
    });
  });
  ...
  it("revokes one TUI without changing another TUI's principal", async () => {
    const appRoot = await createTemporaryDirectory();
    const handle = await startServer(appRoot);
    const server = LocalDevelopmentAuthServer.fromMetadata({
      appRoot,
      metadata: handle.metadata,
    });
    const firstGrant = resultValue(await server.create({ userId: "vercel-user-a" }));
    const secondGrant = resultValue(await server.create({ userId: "vercel-user-b" }));

    await firstGrant.dispose();

    await expect(server.read(firstGrant.token)).resolves.toEqual({ ok: true, value: undefined });
    await expect(server.read(secondGrant.token)).resolves.toMatchObject({
      ok: true,
  ...
  it("returns an error when another local auth server is active in this process", async () => {
    const firstAppRoot = await createTemporaryDirectory();
    const secondAppRoot = await createTemporaryDirectory();
    const firstServer = await startServer(firstAppRoot);

    await expect(LocalDevelopmentAuthServer.start(secondAppRoot)).resolves.toEqual({
      ok: false,
      error: {
        kind: "already-active",
        serverInstanceId: firstServer.metadata.serverInstanceId,
      },
    });
  });
  ...
  it("returns an error for an invalid user id", async () => {
    ...
    await expect(server.create({ userId: " " })).resolves.toEqual({
      ok: false,

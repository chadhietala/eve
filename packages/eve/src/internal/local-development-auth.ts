...
export type LocalDevelopmentAuthStartError =
...
{
  static #activeServerInstanceId: string | undefined;
  ...
  /** Rehydrates registry access from trusted project metadata. */
  static fromMetadata(input: {
  ...
  /** Rehydrates registry access from the active dev server's process environment. */
  static fromEnvironment(): LocalDevelopmentAuthServer | undefined {
  ...
  static async start(
    ...
    if (LocalDevelopmentAuthServer.#activeServerInstanceId !== undefined) {
      return err({
        kind: "already-active",
        serverInstanceId: LocalDevelopmentAuthServer.#activeServerInstanceId,
      });
    }
    ...
    LocalDevelopmentAuthServer.#activeServerInstanceId = metadata.serverInstanceId;
    ...
    const previousDirectory = process.env[EVE_LOCAL_DEV_AUTH_DIRECTORY_ENV];
    const previousInstanceId = process.env[EVE_LOCAL_DEV_AUTH_INSTANCE_ID_ENV];
    ...
    try {
      ...
      process.env[EVE_LOCAL_DEV_AUTH_DIRECTORY_ENV] = directory;
      process.env[EVE_LOCAL_DEV_AUTH_INSTANCE_ID_ENV] = metadata.serverInstanceId;
    } catch (cause) {
      LocalDevelopmentAuthServer.#activeServerInstanceId = undefined;
      ...
      async dispose() {
        if (disposePromise === undefined) {
          disposePromise = (async () => {
            if (process.env[EVE_LOCAL_DEV_AUTH_DIRECTORY_ENV] === directory) {
              restoreEnvironmentValue(EVE_LOCAL_DEV_AUTH_DIRECTORY_ENV, previousDirectory);
            }
            if (process.env[EVE_LOCAL_DEV_AUTH_INSTANCE_ID_ENV] === metadata.serverInstanceId) {
              restoreEnvironmentValue(EVE_LOCAL_DEV_AUTH_INSTANCE_ID_ENV, previousInstanceId);
            }
            await rm(directory, { force: true, recursive: true });
            if (LocalDevelopmentAuthServer.#activeServerInstanceId === metadata.serverInstanceId) {
              LocalDevelopmentAuthServer.#activeServerInstanceId = undefined;
            }
          })().catch((error: unknown) => {

...
interface StartDevelopmentServerOptions {
...
export function startDevelopmentServer(
  rootDir: string,
  options?: StartDevelopmentServerOptions & { existing?: "reject" },
): Promise<StartedDevelopmentServer>;
export function startDevelopmentServer(
  rootDir: string,
  options?: StartDevelopmentServerOptions,
...
export async function startDevelopmentServer(
  ...
  try {
    const preparedHost = await prepareApplicationHost(project.appRoot, { dev: true });

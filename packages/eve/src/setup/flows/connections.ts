...
export const CONNECTIONS_PROMPT_MESSAGE = "What should your agent connect to?";
...
export interface ConnectionsFlowDeps {
  ...
  listPendingConnectConnections: typeof listPendingConnectConnections;
...
function connectionListRows(
  ...
  for (const entry of CONNECTION_CATALOG) {
    if (authored.has(entry.slug) && !pending.has(entry.slug)) {
      rows.push({
        value: entry.slug,
        label: entry.label,
        completed: true,
...
async function pickConnection(
  ...
  const rows = connectionListRows(authored, pending, projectLinked, authStatus);
...
export async function runConnectionsFlow(input: {
  ...
  const [deployment, initialConnections, initialPendingConnections, authStatus] =
    await checkProject(() =>
      Promise.all([
        deps.detectDeployment(appRoot, { signal }),
        deps.listAuthoredConnections(appRoot),
        deps.listPendingConnectConnections(appRoot),
  ...
  let pending = new Set(initialPendingConnections);
  ...
  while (true) {
    ...
    if (pick === "done") break;
    if ((authored.has(pick) && !pending.has(pick)) || isCatalogEntry(pick) === undefined) continue;
    ...
    try {
      ...
      const [observed, observedPending] = await checkProject(() =>
        Promise.all([
          deps.listAuthoredConnections(appRoot),
          deps.listPendingConnectConnections(appRoot),
        ]),
      ...
      if (!authored.has(pick) && observed.includes(pick)) {
        ...
        pending = new Set(observedPending);
        retainedFailures.set(pick, toErrorMessage(error));
        if (signal?.aborted === true) break;
        continue;
      }
      if (authored.has(pick) && pending.has(pick)) {
        pending = new Set(observedPending);
        retainedFailures.set(pick, toErrorMessage(error));
        if (signal?.aborted === true) break;
        continue;

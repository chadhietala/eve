...
Authenticates requests only when they are addressed to a loopback hostname (`localhost`, `*.localhost`, `127.0.0.0/8`, or `::1`). Each local TUI resolves the logged-in Vercel CLI user's stable id and registers a server-lifetime grant under `.eve/dev-auth/`; on POSIX systems, eve restricts the server and grant directories to the current OS user. The request carries only the grant's random token. `localDev()` resolves that token on each request and returns a `user` principal, so Connect-backed connections can complete and reuse that user's interactive authorization. Other local requests keep the synthetic `local-dev` principal.

The grant is bound to one dev-server run and one TUI. `/login` rotates it when the CLI user changes, and a TUI that uses the copyable local-server URL registers its own grant. A normal TUI exit revokes its grant; if the TUI is terminated abruptly, the token remains valid until that dev server exits. Worker reloads do not carry identity state because the mapping lives outside the worker environment. The registry trusts processes running as the same local OS user; it is not a production authentication mechanism.

The host check keys off the request URL's hostname rather than the bare `process.env.VERCEL` flag, and that's deliberate: a deployment outside Vercel leaves `VERCEL` unset, so sniffing that flag alone would wave through all public traffic. There's one process-level exception. `vercel dev`, detected by `VERCEL=1` and `VERCEL_ENV=development` together, opens the local dev server even when it serves over a non-loopback host. Every other non-loopback request returns `null` and falls through.

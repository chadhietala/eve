...
The string passed to `connect()` is the connector UID. `/connect` creates or reuses that connector, attaches it to the linked Vercel project, and writes the UID into this file. At runtime, eve requests a connector token for the active user. If Connect requires authorization, eve emits `authorization.required`, parks the turn, and resumes it after the user finishes the browser flow.

The full setup and principal model live in [Auth & route protection](./guides/auth-and-route-protection).
